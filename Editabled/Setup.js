...
editors.map(function(index) {
	...
	var extraCanvases = _.range(6).map(function() {
	...
	writers.uiGhost =       extraCanvases[5][0].getContext('2d'); //If the pencil is obscured by drawing something, it should appear here as a ghostly purple outline. This lets you get 'inside' the picture's layers. Purple intensity is related to alpha of overlay. This layer is not moved when the canvas is.
	writers.staticWidgets = extraCanvases[4][0].getContext('2d'); //Output of 'above this layer' from pixel store.
	writers.overlay =       extraCanvases[3][0].getContext('2d'); //Output of 'above this layer' from pixel store.
	writers.ui =            canvas.getContext('2d');              //The preview object for the mouse is written here. This layer is not moved when the canvas is.
	writers.drawCache =     extraCanvases[2][0].getContext('2d'); //The 'fast render', to be ereased by the pixel store update event, is written here. This is a preview of sorts for the action that the pixel store will take.
